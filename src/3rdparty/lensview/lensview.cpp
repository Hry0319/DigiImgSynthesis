...
static float lvFStop  = -1; 
static float lviPlaneZ = INFINITY;  
static float lviPlaneY = INFINITY;
static LENSMODE lvMode = PRECISE; 
...
//export to lensdata.cpp for show the ray Chain				   
...
static void lvDisplay(void) {
	//before we display the lsystem, the initialized has been done in main();
	...
	//Draw the text on the bargin.

	//10. Deal with the parameters. 	
	float cur_aperture = lsystem.calAperture(lvFStop);	

	char strMode[10]; 
	if (lvMode== PRECISE)  sprintf(strMode, "Precise");
	else if(lvMode== THICK)  sprintf(strMode, "THICK");

	float oPlane = lsystem.caloPlane(lviPlaneZ); 
    glColor3f(1, 1, 1);
	lvText(9, lvHeight-16, SBFONT, 0, "fstop %1.1f (aperture %1.1fmm) | "
		                              "object %1.1fmm | image %1.1fmm | mode: %s", 
									  lsystem.fstop, cur_aperture,oPlane ,lviPlaneZ, strMode ); 

	Point ep = lsystem.calExitPupil(); 
	glColor3f(1, 1, 1);
	lvText(9, 8, SBFONT, 0, "F %1.1fmm | F' %1.1fmm | P %1.1fmm | "
		                    "P' %1.1fmm | exit pupil (%1.1fmm, %1.1fmm)", 
							lsystem.f1, lsystem.f2, lsystem.p1, lsystem.p2, ep.z , ep.y * 2);

	
	//20.  Draw the bunch of rays on it
	
	lsystem.DrawRaysFromiPlane(lviPlaneY, lviPlaneZ, lvMode );

	//30. Set one point in the image, and draw all the rayChains from this point
	glPopMatrix();
	glutSwapBuffers();
}
...
static void lvMotion(int x, int y) {
	...
	if (lvPressed[GLUT_MIDDLE_BUTTON]) {
		...
		lviPlaneZ += float(lastx - x)*2.0f*norm;  
		if (lviPlaneZ >= lsystem.f2 - 1) lviPlaneZ = lsystem.f2 - 1; 
		lviPlaneY += float(lasty - y)*2.0f*norm;
	}
...
static void lvKey(unsigned char key, int x, int y) {
    switch (key) {
		...
		case 'm': // toggle trace mode
			if (lvMode == PRECISE) lvMode = THICK; 
			else lvMode = PRECISE;
			break;
		case '-': // decrease fstop
			if (lvFStop > lsystem.minFS) lvFStop -= 0.1;
			break;
		case '=': // increase fstop
			if (lvFStop < lsystem.maxFS) lvFStop += 0.1;
			break;	
		default: break;
	}
	glutPostRedisplay();
...
static void init (){
	printf("---------------------------------------------\n");
	printf("A simple realistic camera lens raytracer.\n");
	printf("tenghui zhu\n");

	printf("[Key controls]\n");
	printf("r: snap the ray origin to the axial point\n");
	printf("d: toggle display of traced rays\n");
	printf("m: toggle between exact and thick lens trace\n");
	printf("=: increase fstop\n");
	printf("-: decrease fstop\n");

	printf("[Mouse controls]\n");
	printf("left:   translate OpenGL camera in x and y\n");
	printf("middle: left-right changes camera focus\n");
	printf("up-down moves point on image plane\n");
	printf("right:  zoom to scale size of camera lens (x)\n");
	printf("---------------------------------------------\n");
}
